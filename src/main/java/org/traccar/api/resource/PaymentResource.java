...
public class PaymentResource extends BaseResource {
    ...
    private void processSubscription(String email, String provider, String planType) throws StorageException {
        ...
        if (user != null) {
            ...
            // Pour WayTracker, on ajoute 1 an (ou Calendar.MONTH, 1 pour un mois)
            ...
            // --- LOGIQUE DYNAMIQUE DES PACKS WayTracker ---
            ...
            Group fleetGroup = storage.getObjects(Group.class, new Request(
                    new Columns.All(),
                    new Condition.Equals("name", "WayTracker")))
                    .stream().findFirst().orElse(null);
            ...
            if (fleetGroup != null) {
                try {
                    ...
                    // Log pour confirmer
                    LOGGER.info("⚠️ Utilisateur " + email + " retiré du groupe public WayTracker (Abonnement actif)");
                } catch (Exception e) {
    ...
    private void sendEmail(User user, String endDate, String planName) {
        if (mailManager != null) {
            try {
                String subject = "Bienvenue sur WayTracker - Votre abonnement " + planName + " est actif !";
                String body = "Bonjour " + user.getName() + ",\n\n"
                        + "Votre paiement a été validé avec succès.\n"
                        + "Votre accès à WayTracker est désormais actif jusqu'au " + endDate + ".\n\n"
